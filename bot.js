...
// Constant headers to avoid re-allocation during API calls
...
export class AapClient {
  constructor(card) {
    this.card = card;
    // Pre-calculating frequently used fields for performance
    this.cardId = card.agent_id;
    this.valuesApplied = card.values?.upholds || [];
    // Session-prefixed counter for fast, safe trace ID generation
  ...
  async traceAction(opts) {
    const trace = {
      // Using session-prefixed counter for performance
      trace_id: `tr-${this.sessionPrefix}-${(++this.counter)}`,
      card_id: this.cardId,
